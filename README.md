...
| 数据与运营 | 已完成真实试用链路 | Tushare 基金净值、统一估值序列、生产管道、租约调度、Prometheus 指标、可选 OpenTelemetry 和告警 |
| API 与界面 | 已完成 | 真实试用产品仪表盘、产品切换、组合/基准双净值曲线、历史模拟披露和 AI 运营指标 |
| 安全与审计 | 已完成基础版本 | operator/admin 权限、操作审计、哈希链、导出和保留策略 |
| 灾难恢复 | 已完成 | 在线备份、清单校验、受控恢复和非破坏性恢复演练 |
| PostgreSQL 迁移 | 进行中 | 版本 15 增量升级/回滚、数据搬迁、切换演练与 CI PostgreSQL 16 验收 |
| 部署门禁 | 已完成基础版本 | schema、数据、审计、密钥、只读状态、备份新鲜度和可观测性预检 |
| PostgreSQL 灾备 | 已完成工具链 | 原生 custom-format 备份、SHA-256 清单、空库恢复与 CI 演练 |
| 自动化验证 | 已完成 | 201 项测试、82% 分支覆盖率、20 年历史性能基准和 GitHub Actions |
...
- 使用获得授权的生产行情替换 ETF 代理数据；
- 补齐手续费、滑点、交易日、汇率等真实回测条件；
- 在实际服务器验证 Docker、定时任务、告警、备份与恢复；
- 接入企业身份认证或 SSO，并根据并发需求评估迁移至 PostgreSQL；
- 完成业务、数据授权、产品说明书及合规审核；
- 通过持续前向模拟验证策略稳定性。
...
PostgreSQL 迁移正在分阶段推进。API 已具备数据库 URL 切换和基础 PostgreSQL
集成验收，但现有 SQLite 数据尚未搬迁；迁移状态和切换前置条件见
[PostgreSQL 迁移手册](docs/POSTGRESQL_MIGRATION.md)。

组合版本会在其生效日开始参与下一收益区间计算，系统支持跨版本连续净值、组合与基准完整净值曲线持久化、基准归一化、超额收益及绩效快照持久化。仪表盘会同时绘制组合和基准曲线。
...
生产环境可设置 `FUNDOS_API_KEY`，写接口随后要求 `X-API-Key` 请求头。产品创建和组合发布支持 `Idempotency-Key`，相同请求重复提交会返回首次结果，不会重复写入。业务写入与幂等结果在同一 SQLite 写事务中提交，并发重试只执行一次业务动作。
